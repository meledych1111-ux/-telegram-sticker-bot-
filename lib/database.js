...
async function saveSticker(chatId, format, stickerSize, processingTime, stickerUrl = null) {
  await init();
  const [user] = await sql`SELECT id FROM users WHERE chat_id = ${chatId}`;
  if (!user) return;

  await sql`
    INSERT INTO stickers (user_id, original_format, sticker_size, processing_time, sticker_url)
    VALUES (${user.id}, ${format}, ${stickerSize}, ${processingTime}, ${stickerUrl})`;
