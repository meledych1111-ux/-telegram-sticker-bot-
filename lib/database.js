// 🗄️  lib/database.js  –  без циклических зависимостей, готов к Vercel
...
async function init() {
  ...
  ready = true;
  console.log('✅  БД готова (каскадное удаление, BIGINT, без циклических зависимостей)');
}
...
async function createCollection(chatId, name, description = '') {
  await init();
  const [user] = await sql`SELECT id FROM users WHERE chat_id = ${chatId}`;
  if (!user) throw new Error('Пользователь не найденен');
...
async function deleteCollection(chatId, collectionId) {
  await init();
  /* стикеры внутри удалятся автоматически (ON DELETE CASCADE) */
  await sql`
