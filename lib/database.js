// 🗄️ БАЗА ДАННЫХ С ПРАВИЛЬНЫМИ НАЗВАНИЯМИ СТОЛБЦОВ
...
async function initializeTables() {
  ...
  try {
    ...
    // Таблица стикеров в подборках (ИСПРАВЛЕННЫЕ СТОЛБЦЫ)
    await sql`
      CREATE TABLE IF NOT EXISTS collection_stickers (
        id SERIAL PRIMARY KEY,
        collection_id INTEGER,
        sticker_data TEXT NOT NULL,  -- ← ПРАВИЛЬНОЕ НАЗВАНИЕ
        added_at TIMESTAMP DEFAULT NOW()
      )
    `;

    // Таблица избранных стикеров (ИСПРАВЛЕННЫЕ СТОЛБЦЫ)
    await sql`
...
// ➕ ДОБАВИТЬ СТИКЕР В ПОДБОРКУ (ИСПРАВЛЕННАЯ - sticker_data вместо sticker_id)
async function addStickerToCollection(collectionId, stickerData) {
  try {
    ...
    `;
    console.log(`✅ Стикер добавлен в подборку ${collectionId}`);
  } catch (error) {
...
// ⭐ ДОБАВИТЬ В ИЗБРАННОЕ (ИСПРАВЛЕННАЯ - sticker_data вместо sticker_id)
