...
export async function initDatabase() {
  ...
  try {
    ...
    await sql`SELECT 1`;
    logger.info('✅ Подключение к Neon PostgreSQL установлено');
...
async function createTables() {
  ...
  try {
    ...
    logger.error(`❌ Ошибка создания таблиц: ${err.message}`);
  }
}
...
export async function getOrCreateUser(telegramUser) {
  ...
  try {
    ...
    if (!user) {
      ...
      logger.info(`👤 Новый пользователь: ${telegramUser.id} (@${telegramUser.username || 'no-username'})`);
      return newUser;
    }
    
    return user;
  } catch (err) {
    logger.error(`❌ Ошибка getOrCreateUser: ${err.message}`);
    throw err;
  }
}
...
export async function saveSticker(stickerData) {
  ...
  try {
    ...
    logger.info(`💾 Стикер сохранен: ${sticker.id}`);
    return sticker;
  } catch (err) {
    logger.error(`❌ Ошибка saveSticker: ${err.message}`);
    throw err;
  }
}
...
export async function getStats(telegramId) {
  ...
  try {
    ...
    logger.error(`❌ Ошибка getStats: ${err.message}`);
    return { stickers_created: 0 };
  }
}
