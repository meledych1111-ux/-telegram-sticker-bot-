// 🗄️ БАЗА ДАННЫХ С ПОДБОРКАМИ И ЭФФЕКТАМИ
  ...
  try {
    ...
    if (users.length > 0) {
      await sql`
        ...
        VALUES (${users[0].id}, ${format}, ${stickerSize}, ${processingTime}, ${effect})
      `;
  ...
  try {
    ...
    if (users.length > 0) {
      const result = await sql`
        ...
        VALUES (${users[0].id}, ${name}, ${description}, ${isPublic})
        RETURNING id
      `;
  ...
  try {
    ...
    if (users.length > 0) {
      await sql`
        ...
        VALUES (${users[0].id}, ${stickerData}, ${effectType})
      `;
...
// 🎭 ПОЛУЧИТЬ ДОСТУПНЫЕ ЭФФЕКТЫ
async function getAvailableEffects(chatId) {
  try {
    await initializeTables();
    
    const user = await sql`
      SELECT premium_level FROM users WHERE chat_id = ${chatId}
    `;
    
    const isPremium = user.length > 0 && user[0].premium_level > 0;
    
    const effects = await sql`
      SELECT * FROM effects 
      WHERE is_premium = false OR ${isPremium} = true
