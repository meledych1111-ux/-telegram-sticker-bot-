...
const { 
  ...
  getUserFavorites,
  getAvailableEffects,
  initializeTables // Добавляем инициализацию
} = require('./database');
  ...
  switch (command.toLowerCase()) {
    ...
    case '🎨 создать стикер':
      await sendMessage(chatId, 
        ...
        '🎭 Можно применить эффекты после создания',
...
async function showEffectsMenu(chatId) {
  try {
    ...
    effects.forEach(effect => {
      message += `• ${effect.name}${effect.is_premium ? ' 💎' : ''} - ${effect.description}\n`;
    });
    ...
    await sendMessage(chatId, 
      ...
      'Отправьте изображение для создания стикера без эффектов.',
      MenuBuilder.getMainMenu()
    );
  }
}
