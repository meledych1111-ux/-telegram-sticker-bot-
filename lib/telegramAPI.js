...
async function handleTextMessage(chatId, text) {
  ...
  switch (command.toLowerCase()) {
    ...
    default:
      // Если это не команда меню, проверяем эффекты
      ...
      if (effect) {
        ...
        if (command.length > 2 && command.length < 50) {
          await handleCollectionCreation(chatId, command);
        } else {
          await sendMainMenu(chatId);
        }
      }
...
// 🔘 ОБРАБОТКА CALLBACK QUERY (ИСПРАВЛЕННАЯ)
async function handleCallbackQuery(callbackQuery) {
  ...
  try {
    if (data.startsWith('fav_')) {
      ...
      // Обновляем сообщение
      await sendMessage(chatId, '✅ Стикер добавлен в избранное!');
    }
      ...
      if (collections.length === 0) {
        ...
        // Показываем подборки для выбора
        ...
        inlineKeyboard.push([{ text: '➕ Новая подборка', callback_data: `create_col_${stickerId}` }]);
      ...
      await sendMessage(chatId, '✅ Стикер добавлен в подборку!');
    }
    else if (data.startsWith('create_col_')) {
      const stickerId = data.split('_')[2];
      await sendMessage(chatId, 
...
// 👋 ПРИВЕТСТВЕННОЕ СООБЩЕНИЕ (ИСПРАВЛЕННОЕ)
...
// 🎭 ПОКАЗАТЬ МЕНЮ ЭФФЕКТОВ (ИСПРАВЛЕННАЯ)
async function showEffectsMenu(chatId) {
  try {
    const effects = await getAvailableEffects(chatId);
    
    let message = '🎭 *Эффекты для стикеров*\n\n';
    message += 'Выберите эффект для следующего стикера:\n';
    ...
    message += '\n📝 *Как использовать:*\n';
    message += '1. Выберите эффект из списка\n';
    message += '2. Отправьте изображение\n';
    message += '3. Получите стикер с эффектом!';
...
async function createNewCollection(chatId) {
  await sendMessage(chatId, 
    ...
    'Введите название для новой подборки:\n\n' +
    '✨ *Примеры:* "Мемы", "Природа", "Портреты", "Путешествия"',
    MenuBuilder.removeMenu()
  );
}
...
// 🖼️ ОБРАБОТКА ФОТОГРАФИЙ (ИСПРАВЛЕННАЯ)
async function handlePhoto(chatId, photos) {
  ...
  try {
    ...
    await sendMessage(chatId, 
      ...
      'Возможно, изображение слишком большое или повреждено.\n' +
      'Попробуйте другой файл.'
    );
  }
}

// 📎 ОБРАБОТКА ДОКУМЕНТОВ (ИСПРАВЛЕННАЯ)
async function handleDocument(chatId, document) {
  ...
  try {
    ...
    await sendMessage(chatId, 
      '❌ Не удалось создать стикер.\n' +
      'Возможно, файл поврежден или слишком большой.\n' +
      'Попробуйте другое изображение.'
    );
  }
}

// 📖 СООБЩЕНИЕ ПОМОЩИ (ИСПРАВЛЕННОЕ)
