  ...
  switch (command.toLowerCase()) {
    ...
    default:
      ...
      if (effect) {
        ...
        // Проверяем если это создание подборки
  ...
  try {
    if (data.startsWith('fav_')) {
      ...
      await sendMessage(chatId, 
        ...
        MenuBuilder.getCollectionsMenu(await getUserCollections(chatId))
      );
    }
...
async function handleCollectionCreation(chatId, collectionName) {
  try {
    ...
    await sendMessage(chatId, '❌ Не удалось создать подборку. Попробуйте другое название.');
  }
}
...
// 🎭 ПОКАЗАТЬ МЕНЮ ЭФФЕКТОВ (ИСПРАВЛЕННАЯ - БЕЗ is_premium)
...
async function showUserFavorites(chatId) {
  try {
    ...
    if (favorites.length === 0) {
      ...
      // Здесь можно показывать превью избранных стикеров
      // await showFavoritePreviews(chatId, favorites);
    }
  } catch (error) {
