// 📞  lib/telegramAPI.js  –  чистый, без папок, 2025-совместимый
...
const userEffects       = new Map();          // chatId → effect
const awaitCollectionName = new Map();        // chatId → true
...
async function handleTextMessage(chatId, text) {
  ...
  switch (t) {
    ...
    default: {
      ...
      if (effects.find(e => e.name.toLowerCase() === t))
        return handleEffectSelection(chatId, t);
      return sendMainMenu(chatId);          // неизвестная команда
    }
  }
}
...
async function handlePhoto(chatId, photos) {
  ...
  try {
    ...
    await saveSticker(chatId, 'photo', sticker.length, time);
    await sendSticker(chatId, sticker);
    userEffects.delete(chatId);

    await sendMessage(chatId, '✅ Стикер готов!', MenuBuilder.getStickerActions(time));
  } catch (e) {
    console.error(e);
...
async function handleDocument(chatId, doc) {
  return handlePhoto(chatId, [{ file_id: doc.file_id }]); // переиспользуем логику
}
...
async function handleCallbackQuery(q) {
  ...
  try {
    if (data.startsWith('fav_')) {
      await addToFavorites(chatId, `sticker_${data.split('_')[1]}`);
      return answerCallbackQuery(q.id, '⭐ В избранном!');
    }

    if (data.startsWith('col_')) {
      const [, stickerId] = data.split('_');
    ...
    if (data.startsWith('add_to_col_')) {
      const [,, colId, stickerId] = data.split('_');
      await addStickerToCollection(colId, `sticker_${stickerId}`);
    ...
    if (data.startsWith('delete_col_')) {
      const colId = data.split('_')[2];
      await deleteCollection(chatId, colId);
      await answerCallbackQuery(q.id, '🗑️ Удалено');
      return showCollectionsMenu(chatId);
    }
  } catch (e) {
    console.error(e);
...
async function showCollectionsMenu(chatId) {
  ...
  await sendMessage(chatId, txt, MenuBuilder.getCollectionsMenu(list));
}
...
async function showUserFavorites(chatId) {
  ...
  await sendMessage(chatId, `⭐ Избранных стикеров: ${favs.length}.`, MenuBuilder.getMainMenu());
}
