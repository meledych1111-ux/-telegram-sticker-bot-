// 📞 ОБРАБОТКА СООБЩЕНИЙ ОТ TELEGRAM API С МЕНЮ И ЭФФЕКТАМИ
...
async function handleTextMessage(chatId, text) {
  ...
  switch (command.toLowerCase()) {
    ...
    // 🆕 КОМАНДЫ МЕНЮ
    case '🎨 создать стикер':
      await sendMessage(chatId, '📷 Отправьте мне изображение для создания стикера!', MenuBuilder.removeMenu());
      break;
    ...
    default:
      ...
      if (await isEffectCommand(chatId, command)) {
        await handleEffectSelection(chatId, command);
      } else {
        await sendMainMenu(chatId);
      }
  }
...
async function createNewCollection(chatId) {
  ...
  // Здесь можно добавить состояние ожидания названия
}

// 🎯 ОБРАБОТКА ВЫБОРА ЭФФЕКТА
async function handleEffectSelection(chatId, effectName) {
  // Сохраняем выбранный эффект для следующего стикера
  // Можно использовать временное хранилище или базу данных
  await sendMessage(chatId, 
...
// 🖼️ Обработка фотографий через Telegram API (ОБНОВЛЕННАЯ)
async function handlePhoto(chatId, photos, effect = 'none') {
  ...
  try {
    ...
    await saveSticker(chatId, 'photo', stickerBuffer.length, processingTime, effect);
...
// 📎 Обработка документов через Telegram API (ОБНОВЛЕННАЯ)
async function handleDocument(chatId, document, effect = 'none') {
  const startTime = Date.now();
  ...
  try {
    ...
    const processingTime = Date.now() - startTime;
    await saveSticker(chatId, 'document', stickerBuffer.length, processingTime, effect);
...
// 👋 Приветственное сообщение (ОБНОВЛЕННОЕ)
...
async function sendHelpMessage(chatId) {
  const message =
    ...
    '• Премиум эффекты для крутого вида\n\n' +
...
// Проверка является ли текст эффектом
async function isEffectCommand(chatId, command) {
  const effects = await getAvailableEffects(chatId);
  return effects.some(effect => 
    effect.name.toLowerCase() === command.toLowerCase()
  );
}
...
async function sendSticker(chatId, stickerBuffer, options = {}) {
  try {
    ...
    const response = await axios.post(`${BOT_URL}/sendSticker`, form, {
      headers: form.getHeaders(),
      ...options
    });

    return response.data.result; // Возвращаем информацию о отправленном стикере
  } catch (error) {
