...
// Конфигурация
const STICKER_SIZE = 512; // Размер стикера в Telegram
...
export async function processImage(imageBuffer, options = {}) {
  try {
    ...
    // Создаем изображение с Sharp
    let image = sharp(imageBuffer);
    
    // Получаем метаданные
    const metadata = await image.metadata();
    await logger.info(`📐 Размер исходного изображения: ${metadata.width}x${metadata.height}, формат: ${metadata.format}`);
    ...
    await logger.info(`✅ Изображение обработано за ${processingTime}ms`);
    
    return processedBuffer;
  } catch (error) {
    await logger.error(`❌ Ошибка обработки изображения: ${error.message}`);
    throw error;
  }
}
...
export async function validateImage(imageBuffer) {
  try {
    ...
    await logger.error(`❌ Ошибка валидации изображения: ${error.message}`);
    throw error;
  }
}
