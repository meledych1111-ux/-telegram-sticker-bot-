...
export async function processImage(imageBuffer, options = {}) {
  try {
    ...
    logger.info(`📐 Размер исходного изображения: ${metadata.width}x${metadata.height}, формат: ${metadata.format}`);
    ...
    logger.info(`✅ Изображение обработано за ${processingTime}ms`);
    
    return processedBuffer;
  } catch (err) {
    logger.error(`❌ Ошибка обработки изображения: ${err.message}`);
    throw err;
  }
}
...
export async function validateImage(imageBuffer) {
  try {
    ...
    logger.error(`❌ Ошибка валидации изображения: ${err.message}`);
    throw err;
  }
}
