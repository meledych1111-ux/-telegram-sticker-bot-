...
const COLORS = {
  ...
  gold: 0xFFD700FF,
  silver: 0xC0C0C0FF,
  gradient: 0x6A11CBFF // Для градиента
};
...
async function addFrame(imageBuffer, colorName = 'white') {
  try {
    ...
    const innerFrameColor = 0x000000FF;
...
async function addPearlEffect(imageBuffer) {
  try {
    ...
    image.scan(0, 0, image.bitmap.width, image.bitmap.height, function(x, y, idx) {
      ...
      this.bitmap.data[idx + 1] = Math.min(255, this.bitmap.data[idx + 1] * 1.1);
      this.bitmap.data[idx + 2] = Math.min(255, this.bitmap.data[idx + 2] * 1.1);
    });
    
    image.blur(1);
    console.log('✅ Перламутровый эффект добавлен');
    return await image.getBufferAsync(Jimp.MIME_PNG);
  } catch (error) {
    console.error('❌ Ошибка addPearlEffect:', error);
    return imageBuffer;
  }
}

// Градиентный эффект
async function addGradientEffect(imageBuffer) {
  try {
    const image = await Jimp.read(imageBuffer);
    ...
    console.log('✅ Градиентный эффект добавлен');
    return await image.getBufferAsync(Jimp.MIME_PNG);
  } catch (error) {
