...
// 📊 Сохранить пользователя (ИСПРАВЛЕННАЯ)
...
// 🎨 Сохранить информацию о стикере (ИСПРАВЛЕННАЯ)
...
async function getAvailableEffects(chatId) {
  try {
    ...
    // Просто возвращаем все эффекты для простоты
...
async function getUserStats(chatId) {
  try {
    ...
    const user = await sql`
      SELECT stickers_created FROM users WHERE chat_id = ${chatId}
    `;
